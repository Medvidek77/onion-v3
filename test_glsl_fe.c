...
void fe_add(int h[10], const int f[10], const int g[10]) {
    for (int i=0; i<10; i++) h[i] = f[i] + g[i];
}
void fe_mul(int h[10], const int f[10], const int g[10]) {
...
void fe_sq(int h[10], const int f[10]) {
    int64_t f0 = f[0], f1 = f[1], f2 = f[2], f3 = f[3], f4 = f[4];
    int64_t f5 = f[5], f6 = f[6], f7 = f[7], f8 = f[8], f9 = f[9];
...
void fe_invert(int out_fe[10], const int z[10]) {
    int t0[10], t1[10], t2[10], t3[10];

    fe_sq(t0, z);

    fe_sq(t1, t0);
    for (int i = 1; i < 2; ++i) { fe_sq(t1, t1); }
    fe_mul(t1, z, t1);
    fe_mul(t0, t0, t1);

    fe_sq(t2, t0);
    for (int i = 1; i < 1; ++i) { fe_sq(t2, t2); }
    fe_mul(t1, t1, t2);
